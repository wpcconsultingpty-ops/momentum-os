...
// new slide brand voice: no banned soft-self-help phrasing, every draft ends
// with the CTA, and every draft carries a valid light/dark slide theme.
...
it("ends every caption with the app CTA", async () => {
const { generateDrafts, APP_CTA } = await import("@/lib/content/generate");
const drafts = await generateDrafts(12);
for (const draft of drafts) {
expect(draft.caption.trim().endsWith(APP_CTA)).toBe(true);
}
});
